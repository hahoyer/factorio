    ...
    abstract class TablePathObject : DumpableObject
    {
        ...
        public IObjectView Materialize()
        {
            ...
            try
            {
                ...
                switch(type)
                {
                    case LuaTypes.Boolean: return new ImbeddedObject(Kernel.LuaToBoolean(-1));
                    case LuaTypes.Number: return new ImbeddedObject(Kernel.LuaNetToNumber(-1));
                    ...
                    case LuaTypes.Function: return new FunctionObject(Parent, RootObject, Path);
                    case LuaTypes.UserData: return new UserDataObject(Parent, RootObject, Path);
                    case LuaTypes.Table: return new TableView(Parent, RootObject, Path);
                    case LuaTypes.Nil: return null;
                    default:
                        ;
