...
namespace MmasfUI
{
    public sealed class PositionConfig : IDisposable
    {
        ...
        Rect? Position
        {
            ...
            {
                return Convert
                    (0, null, s => (Rect?) new RectangleConverter().ConvertFromString(s));
            }
        ...
        static Rect EnsureVisible(Rect value)
        {
            var allScreens = System.Windows.Forms.Screen.AllScreens;
            if(allScreens.Any(s => s.Bounds.IntersectsWith(value)))
                return value;

            var closestScreen = System.Windows.Forms.Screen.FromRectangle(value);
            var result = value;
